...
def get_dataloader(dic_list, data_index_list, model_mode, shuf, config):
    ...
    with open(datafile) as f:
        ...
        for line in data_list:
            # print(line)
            seq, peptide, peptide_ss, seq_ss, label, pep_bs, _ = line.strip().split('\t')
            if int(label) == 1:
    ...
    if model_mode == 1:
        ...
        data = [X_pep, X_p, X_SS_pep, X_SS_p, X_2_pep, X_2_p, X_dense_pep, X_dense_p, pep_sequence, prot_sequence, X_pep_mask, X_bs_flag, X_bs, labels]
        return construct_dataset(data, config, shuf)
...
def construct_dataset(data, config, shuf):
    ...
    if model_mode == 1:
        ...
        data_loader = Data.DataLoader(
            MyDataSet1(X_pep, X_p, X_SS_pep, X_SS_p, X_2_pep, X_2_p, X_dense_pep, X_dense_p, X_pep_mask, X_bs_flag, X_bs, labels),
            batch_size=batch_size,
            shuffle=shuf,
            drop_last=False)
...
class MyDataSet1(Data.Dataset):
    def __init__(self, X_pep, X_p, X_SS_pep, X_SS_p, X_2_pep, X_2_p, X_dense_pep, X_dense_p, X_pep_mask, X_bs_flag, X_bs, labels):
        self.X_pep = X_pep
        self.X_p = X_p
        self.X_SS_pep = X_SS_pep
        self.X_SS_p = X_SS_p
    ...
    def __getitem__(self, idx):
        return self.X_pep[idx], self.X_p[idx], self.X_SS_pep[idx], self.X_SS_p[idx], self.X_2_pep[idx], self.X_2_p[idx], self.X_dense_pep[idx], self.X_dense_p[idx], self.X_pep_mask[idx], self.X_bs_flag[idx], self.X_bs[idx], self.labels[idx]
...
def load_data(config, k):
    ...
    model_mode = config.model_mode
    # path_all_data = "data/Dataset_all_new"
    # path_data_index_list = "data/clu_thre_"+str(clu_thre)+"/"+str(setting)
