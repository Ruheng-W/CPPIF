...
import os
import torch
import config_CPPIF, data_loader_cppif, util_metric
import CPPIF
...
def train_model(train_iter, valid_iter, test_iter, model, optimizer, criterion, cri_nonReduce, cri_nonReduce_weight, config, iter_k):
    ...
    for epoch in range(1, config.epoch + 1):
        ...
        for batch in train_iter:
            ...
            pred_binary_label, pred_prot_site, pred_pep_site = model.predict(X_pep, X_p, X_SS_pep, X_SS_p, X_2_pep, X_2_p, X_dense_pep, X_dense_p, prot_seqs, pep_seqs)
...
def model_eval(data_iter, model, criterion, cri_nonReduce, cri_nonReduce_weight, config):
        ...
        for batch in data_iter:
            ...
            if config.model_mode == 1:
                ...
                pred_binary_label, pred_prot_site, pred_pep_site = model.predict(X_pep, X_p, X_SS_pep, X_SS_p, X_2_pep, X_2_p,
                                                                   X_dense_pep, X_dense_p, prot_seqs, pep_seqs)
                ...
                if X_bs_flag[0] == 1:
                    metric_pep, roc_data_pep, prc_data_pep = util_metric.caculate_metric(pep_pred_site, pep_real_site, pep_prob_site)
                ...
                if X_prot_bs_flag[0] == 1:
                    metric_prot, roc_data_prot, prc_data_prot = util_metric.caculate_metric(prot_pred_site, prot_real_site, prot_prob_site)
    ...
    if config.model_mode == 1 :
        ...
        print('Evaluation - loss: {:.6f}  loss_bi: {:.6f}  loss_pep: {:.6f}  loss_prot: {:.6f}  bi_ACC: {:.4f}%({}/{})  peptide site ACC: {:.4f}%({}/{})  protein site ACC: {:.4f}%({}/{})'.format(avg_loss, avg_bi_loss, avg_pep_loss, avg_prot_loss,
                                                                      100 * accuracy,
                                                                      corrects,
                                                                      iter_size,
                                                                       100 * accuracy_site,
...
def k_fold_CV(train_loader_list, valid_loader_list, test_loader_list, config, k):
    ...
    test_metric, test_prc_data, test_metric_site, test_prc_data_site, test_metric_prot_site, test_prc_data_prot_site = train_model(train_iter, valid_iter, test_iter, model, optimizer, criterion, cri_nonReduce, cri_nonReduce_weight, config, iter_k)
...
if __name__ == '__main__':
    ...
    for i in [0]:
        k = i
        train_loader_list, valid_loader_list, test_loader_list = data_loader_cppif.load_data(config, k)
